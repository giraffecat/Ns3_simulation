...
{
  *Stream->GetStream () <<Simulator::Now().GetSeconds()  <<" Change from " << oldVal << " to " << newVal <<std::endl;
  std::cout << Simulator::Now().GetSeconds()  <<" Change from " << oldVal << " to " << newVal <<std::endl;
}

void
PhyRxDrop (std::string context, Ptr<const Packet> packet, WifiPhyRxfailureReason reason)
{
  std::cout <<Simulator::Now().GetSeconds()<< " PHY Rx Drop"<<std::endl;
  *Stream->GetStream () <<Simulator::Now().GetSeconds()<< " PHY Rx Drop"<<std::endl;
...
int main (int argc, char *argv[])
{
  ...
  uint32_t Interval = 1;
  float duration = 12.5;
  std::string propagation = "TwoRayGround";

  CommandLine cmd (__FILE__);
  cmd.AddValue ("propagation", "Propagation model", propagation);
  cmd.Parse(argc,argv); //调用函数Parse
  ...
  if(propagation == "TwoRayGround") {
    channel.AddPropagationLoss ("ns3::TwoRayGroundPropagationLossModel");
  } else if(propagation == "Nakagami") {
    channel.AddPropagationLoss ("ns3::NakagamiPropagationLossModel");
    Config::SetDefault ("ns3::NakagamiPropagationLossModel::Distance1", DoubleValue (35.0));
    Config::SetDefault ("ns3::NakagamiPropagationLossModel::Distance2", DoubleValue (40.0));
    Config::SetDefault ("ns3::NakagamiPropagationLossModel::m1", DoubleValue (0.25));
    Config::SetDefault ("ns3::NakagamiPropagationLossModel::m2", DoubleValue (0.11));
  }
  ...
  Config::Connect ("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyRxDrop", MakeCallback (&PhyRxDrop)); 
