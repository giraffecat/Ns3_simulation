...
TraceCwnd (std::string cwnd_tr_file_name, int nFlows)
{
  ...
  std::string path = "/NodeList/1/$ns3::TcpL4Protocol/SocketList/0/CongestionWindow";
...
AggregateGoodput (std::string Vname  , Ptr<OutputStreamWrapper> stream)
{
  *stream->GetStream () << Vname << " " << aggregateGoodput << std::endl;
  std::cout << "The Aggregate Goodput:" << aggregateGoodput << " bits/s" <<std::endl;
...
int main (int argc, char *argv[])
{
  ...
  for(unsigned int i = 0; i < nFlows; i++){
    ...
    sourceApp[i] = ftp.Install (sources.Get (0));
    sourceApp[i].Start (Seconds (start_time));
    sourceApp[i].Stop (Seconds (stop_time));

    sinkHelper.SetAttribute ("Protocol", TypeIdValue (TcpSocketFactory::GetTypeId ()));
    sinkApp[i]= sinkHelper.Install (sinks.Get (0));
  ...
  for(int i = 0; i < nFlows; i ++) {
    ...
    Ptr<OutputStreamWrapper> GoodputStream = ascii.CreateFileStream (transport_prot + "_" + std::to_string(i)+ "flow_"  + delay + "-Goodput.txt");
    Simulator::Schedule (Seconds (stop_time), &GoodputSampling, i, sinkApp[i],
                        GoodputStream);
  }

  //aggregate goodput
  Ptr<OutputStreamWrapper> aggregateStream = ascii.CreateFileStream (prefix_file_name +"_" + transport_prot +"_"+ std::to_string(nFlows)+ "flow_"+ std::to_string(errorRate) + "-Goodput.txt");
  Simulator::Schedule (Seconds (stop_time), &AggregateGoodput, std::to_string(errorRate),
                      aggregateStream);
  ...
  if (true)
    {
      ...
      Simulator::Schedule (Seconds (start_time + 0.000001), &TraceCwnd, prefix_file_name+"_"+ transport_prot +"_"+ std::to_string(nFlows)+ "flows" +"-cwnd.data", nFlows);
